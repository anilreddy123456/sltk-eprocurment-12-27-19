...
public class PoheaderController {
	...
	public String createHeader(@Valid @RequestBody List<PoheaderSapEntity> header) throws ParseException {
		...
		System.out.println("List size: " + header.size());
		...
		for (PoheaderSapEntity poheaderSapEntity : header) {
			...
			for (PolineitemsSapEntity polineitemsSapEntity : polineitemsSapEntities) {
				...
				// poLineItemDAO.getPoLineItems(Long.parseLong(poheaderSapEntity.getPonumber()),
				// Integer.parseInt(polineitemsSapEntity.getPolineid()));
				...
				List<PolineServiceSapEntity> polineserviceSapEntities =polineitemsSapEntity.getPoservice();
				...
				for (PolineServiceSapEntity polineItemServiceSApEntity : polineserviceSapEntities) {
					...
					itemServices.setNetValue(new BigDecimal(polineItemServiceSApEntity.getNet_value()));
 ...
 * List<Poheader> headerlist=new ArrayList<Poheader>(); List<Polineitems>
 * polineitemslist= new ArrayList<Polineitems>();
 * 
 * for (PoheaderSapEntity poheaderSapEntity : header) { Poheader header1= new
 * Poheader();
 * 
 ...
 * //header1.setCreatedDate(new
 * SimpleDateFormat().parse(poheaderSapEntity.getCreateddate()));
 * header1.setCreatedDate(poheaderSapEntity.getCreateddate());
 * header1.setLastModifiedBy(poheaderSapEntity.getLastmodifiedby());
 * //header1.setLastModifiedDate(new
 * SimpleDateFormat().parse(poheaderSapEntity.getLastmodifieddate()));
 ...
 * 
 * for (PolineitemsSapEntity poitementity : lineitem) { Polineitems lineitems1=
 * new Polineitems();
 * 
 ...
 * lineitems1.setUom(poitementity.getUom()); lineitems1.setBalanceAsnQty(new
 * BigDecimal(orderQty)); lineitems1.setPreviousAsnQty(new BigDecimal(0));
 * lineitems1.setGrossValue(lineitems1.getNetValue().multiply(new
 * BigDecimal(orderQty))); lineitems1.setSapTaxCode(poitementity.getTaxcode());
 * lineitems1.setPonumber(new Poheader(poitementity.getPonumber()));
 * 
 * List<PolineTaxSapEntity> polinesaptax=poitementity.getPotax(); for
 * (PolineTaxSapEntity polineitemssaptax : polinesaptax) {
 * 
 ...
 * header1.setPolineitemsList(polineitemslist); headerlist.add(header1); }
 * poheaderDao.saveAll(headerlist);
 * 
 * System.out.println(header.toString());
 **/
